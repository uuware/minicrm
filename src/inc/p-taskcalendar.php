...
$sql = "SELECT t.taskid, t.todo, t.duedate AS duedate, t.contact, t.status FROM #__task t WHERE t.userid='$userid' AND t.duedate>'$sfrom' AND t.duedate<'$sto' ORDER BY duedate";
$db->setQuery($sql);
...
if($lst && count($lst) > 0) {
	foreach($lst as $row) {
		...
		$txt0 = $todo.', '.$status_s.', '.$row->contact;
...
for($ii = 0; $ii < 12/$mline; $ii++) {
	...
	for($jj = 1; $jj <= $mline; $jj++) {
		...
		$out .= '<table class="xcal" border="1" align="center" cellpadding="1" cellspacing="2" style="width:100%;font-size:10pt;text-align:center;border-top:3px double #ccc;">';
...
function madd() {
	byId('p-uedit').innerHTML = '<iframe src="?m=t&act=add&ymd='+g_add_ymd+'" width="100%" height="100%"></iframe>';
	var ftab_edit = FTab('ftab_edit0', 100, 100, 580, 410, 'scroll:0;tab:0;fixed:0;center:1;cookie:0;');
	ftab_edit.show();
...
function medt(id) {
	...
	byId('p-uedit').innerHTML = '<iframe src="?m=t&act=edit&taskid='+id+'" width="100%" height="100%"></iframe>';
	var ftab_edit = FTab('ftab_edit0', 100, 100, 580, 410, 'scroll:0;tab:0;fixed:0;center:1;cookie:0;');
	ftab_edit.show();
	ftab_edit.moveCenter();
	return false;
